...
class AnnouncementListResource(Resource):
    ...
    def get(self):
        ...
        total_count, results = cache_notice.get_announcements_by_page(page, per_page)
...
class AnnouncementResource(Resource):
    ...
    def get(self, target):
        ...
        获取系统公告
        """
        exist = cache_notice.determine_announcement_exist(target)
        if not exist:
            return {'message': 'Invalid announcement.'}, 400

        announcement = cache_notice.get_announcement_detail(target)
        return announcement
