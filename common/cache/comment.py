...
def get_comments_by_article(article_id, offset, limit):
    ...
    # 评论的缓存统计数据
    figure = r_comm_cache.hgetall('a:comm:{}:figure'.format(article_id))
    ...
    if not figure:
        ...
        if end_comment is None:
            # 没有评论数据
            # 设置缓存
            pl.zadd('a:comm', {article_id: timestamp})
            pl.hset('a:comm:{}:figure'.format(article_id), 'count', 0)
            pl.execute()
            return result
        else:
            ...
            pl.zadd('a:comm', {article_id: timestamp})
            pl.hmset('a:comm:{}:figure'.format(article_id), {'count': total_count, 'end_id': end_id})
            pl.execute()
        ...
        # 缓存了的总数量
        cache_count = r_comm_cache.zcard('a:comm:{}'.format(article_id))
